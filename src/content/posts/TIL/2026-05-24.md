...
title: "[TIL] Triton 2D convolution kernel memory pattern"
featured: false
draft: false
tags:
  - TIL
description: "Reasoning through a simple Triton 2D convolution kernel, its output tiling strategy, and the memory access pattern created by looping over kernel elements."
---
...
def conv_2d(
    ...
    output_mask = (
        (output_row_offsets < input_rows - kernel_rows + 1)[:, None]
    ...
    for i in range(kernel_rows):
        for j in range(kernel_cols):
            ...
            input_mask = (
                (input_row_offsets < input_rows)[:, None]
                & (input_col_offsets < input_cols)[None, :]
            )
            input_val = tl.load(
                input
