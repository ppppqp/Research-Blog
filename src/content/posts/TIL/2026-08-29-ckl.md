...
## 4. From layout differences to executable communication
...
Well, I have to admit this is pretty ugly and not fun to write. In future, the `Port` and `acc_distribution` should be inferred and optimized by the compiler naturally, so we can focus on just high-level implementation, and expose a `Triton`-like API.

## 6. Graph-wide realization selection

For a sequence of kernel invocations, CKL models them as graph nodes. Two invocations of the same task may select different alternatives. Vertices are invocations and edges identify producer and consumer dependencies. The implementation right now brute-force searches all potential combination to find the optimal realization selection.
