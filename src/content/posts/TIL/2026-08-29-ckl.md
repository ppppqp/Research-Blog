...
At the logical level, composition is easy:

```text
quantized tile ── dequantize ──> f16 tile ── matrix multiply ──> accumulator tile
```
...
$$\sum{\text{ExecutionCost}(rv)} + \sum{\text{ConversionCost(c_e)}}$$
